...
while True:
    ...
    #tick - allows proper promts to display based on conditions.
    tick = True
    ...
    # Shouldn't ask to take item if invalid move ?
    #take_command = input('\nTake item:')
    if tick == True:
        take_command = input('\nTake item: ')
    ...
    """
    If take_command is yes then activate pickup()
    IF 'Invalid move' or invalid input then re submit move_command ->
    """
    def pickup():
            if rooms[room_position]['item'] not in inventory:
                if rooms[room_position]['item'] != 'no item':
    ...
    if take_command == 'yes' and tick == True:
        pickup()
    else:
        pass
    #user_item = 'item'
